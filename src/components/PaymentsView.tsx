...
export const PaymentsView: React.FC<PaymentsViewProps> = ({
  ...
  const getStatusBadge = (status: InvoiceStatus) => {
    switch (status) {
      case 'paid':
        return <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[11px] font-mono font-medium whitespace-nowrap inline-block">Đã thanh toán</span>;
      case 'pending':
        return <span className="px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[11px] font-mono font-medium whitespace-nowrap inline-block">Chờ thanh toán</span>;
      case 'overdue':
        return <span className="px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-400 border border-rose-500/30 text-[11px] font-mono font-medium whitespace-nowrap inline-block">Quá hạn nợ</span>;
    }
  };
              ...
              {filteredInvoices.map(inv => (
                <tr key={inv.id} className="hover:bg-slate-900/40 transition-colors">
                  ...
                  <td className="p-4 cursor-pointer" onClick={() => onSelectUnit(inv.unitId)}>
                    ...
                    <div className="text-slate-500 text-[10px]">{inv.unitId}</div>
                  </td>
                  ...
                  <td className="p-4 font-bold text-emerald-400 text-sm whitespace-nowrap">
                  ...
                  <td className="p-4 text-right whitespace-nowrap">
                    {inv.status !== 'paid' ? (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 text-[11px] font-mono transition-all font-semibold whitespace-nowrap"
                      >
                        <Check className="w-3.5 h-3.5" />
                        <span>Thu Tiền</span>
                      </button>
                    ) : (
                      <span className="text-[11px] text-slate-500 font-mono whitespace-nowrap">Hoàn tất</span>
                    )}
                  </td>
                </tr>
              ))}
